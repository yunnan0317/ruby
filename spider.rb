...
class Spider
  ...
  def crawl_domain(url, page_limit = 100)
    return @already_visited if @already_visited.size == page_limit

    # url_object是一个url临时文件
    url_object = open_url(url)
  ...
  def open_url(url)
    # open是open-uri中的一个方法, 打开一个url地址, 返回一个临时文件
    open(url)
  rescue
    puts "Unable to open url: " + url.to_s
  end
  ...
  def parse_url(url_object)
    ...
    doc = Hpricot(url_object) # try nokogiri
  ...
  def find_urls_on_page(parsed_doc, current_url)
    parsed_doc.search('a[@href]').each_with_object([]) do |x, urls_list|
      ...
      if new_url
        ...
        new_url = make_absolut(current_url, new_url) if relative?(new_url)
        if new_url
